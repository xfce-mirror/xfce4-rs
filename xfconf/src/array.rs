...
///
/// You can convert between an `Array<T>` and `Vec<T>` using `::from()` and `.into().
...
unsafe impl<'a, T: ValueType + StaticType> FromValue<'a> for Array<T> {
    ...
    unsafe fn from_value(value: &'a glib::Value) -> Self {
        if value.is_type(*G_TYPE_PTR_ARRAY) {
            ...
            // boxed type.
            let array = unsafe { glib::gobject_ffi::g_value_get_boxed(gvalue.0) } as *mut glib::ffi::GPtrArray;
            ...
            if array.is_null() {
                ...
                if !items.is_null() {
                    ...
                    assert!(n_items as isize + size_of::<*mut glib::gobject_ffi::GValue>() as isize <= isize::MAX - items as isize);
